...
for (let u = 0; u < graduates.length; u++) {
  if(graduates[u].university.includes("Uni")){
    uni.push(graduates[u].university);
  }  
}
...
populationTotal = zooAnimals.reduce(function(totalPopulation, zooAnimals){
...
largerPopulation = zooAnimals.filter( zooAnimals => {
    return zooAnimals.population < 5;
});
...
populationTotal = zooAnimals.reduce((totalPopulation, zooAnimals) => {
  return totalPopulation += zooAnimals.population;
}, 0);
